...
- **`fact_join_fanout_check` exists because dbt's built-in tests don't cover this**: `unique`/`not_null`/`relationships` already cover fact-key uniqueness and orphaned-key referential integrity well — a bespoke "no orphaned keys" macro would have been mostly redundant with `relationships`. The genuine gap is join fanout (a dimension matching more than one row for a given fact_key/timestamp, silently duplicating fact rows) — confirmed with the user before building, specifically to avoid reinventing existing generic tests.
- **Failure-detection verification methodology**: for every generic test in this package, the "does it actually catch the bug" path is verified once with a temporary seed (+ scratch model, where needed) + wired test, confirmed to FAIL, then the fixture is fully removed (model, yml, seeds, and the orphaned Snowflake objects they created). Don't re-add a permanently-failing fixture to the suite — a clean `dbt build` with zero failures is the standing bar for this repo. Repeat this same temporary-fixture pattern for any future generic test's negative-case verification.
- **`no_overlapping_scd2_periods` complements `one_current_record_per_key`**, it doesn't replace it: the former checks the *current* row only (no duplicate "current" version); the latter checks the *entire history* for a business key (no two versions' validity windows overlap, at any point in time, current or not). A dimension can pass one and fail the other — write both tests on a Type-2 dimension, not just one.
- **`surrogate_key_is_deterministic` has the same Type-2 composite-key footgun as `generate_dimension()`/`surrogate_key()`**: `business_key` passed to the test must be the exact same key (natural key alone for Type-1, natural key + version discriminator for Type-2) used to actually generate the surrogate key, not just the natural key. Passing only the natural key against a Type-2 dimension produces a false failure — every legitimate historical version has a different (correct) surrogate key for the same natural key. Verified both instances (Type-1 and Type-2, composite key) against real fixtures in the integration suite — don't collapse them to one test case.
